...
class HogThread(threading.Thread):
    def __init__(self, pic_list: [], plt_list: []):
    ...
    def run(self):
        for pic in self.pic_list:
            ...
            hog = HogDescriptor(gray, cell_size=8, bin_size=8)
            vector, image = hog.extract()
            self.plt_list.append((vector, image))
...
hog = HogDescriptor(img, cell_size=8, bin_size=8)
vector, image = hog.extract()
print(np.array(vector).shape)
plt.imshow(image, cmap=plt.cm.gray)
plt.show()
